...
#TASK1
p <- matrix(AirPassengers, ncol = 12, byrow =TRUE,  dimnames = list( as.character(1949:1960),month.abb))
Max <- max(p)
ind = which(p==Max, arr.ind = TRUE)
sprintf("The most profitable month is %s and year is %s", colnames(p)[ind[2]], rownames(p)[ind[1]])
...
sprintf("The maximum revenue by is %s which is in %s %s", Max_month, colnames(month_result)[ind1[2]], rownames(month_result)[ind1[1]])
